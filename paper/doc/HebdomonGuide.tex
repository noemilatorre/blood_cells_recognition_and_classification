...
\Section{Introduzione}
Come primo passo preliminare allo sviluppo del sistema completo, è stato effettuato un test esplorativo utilizzando il modello \hlight{YOLO} (You Only Look Once), una delle architetture più diffuse per il rilevamento e la classificazione di oggetti in immagini. L’obiettivo di questa prova iniziale era quello di valutare la complessità del dataset e ottenere un’indicazione generale sulla difficoltà del compito di classificazione delle cellule, prima di procedere con pipeline più specializzate.
...
Per comprendere meglio la distribuzione delle sole cellule infette, abbiamo temporaneamente escluso i globuli rossi dal training, filtrando manualmente le etichette, e sostituito la PCA con la tecnica \hlight{SelectKBest} per la selezione delle feature. Questo approccio ha portato a un significativo miglioramento delle performance: utilizzando un classificatore \hlight{SVM} si è raggiunto un \hlight{F1-score dell’89\%}, da come è possibile evincere in figura \ref{fig:lda_infette}, che mostra la distribuzione delle classi delle cellule infette dopo l’applicazione di LDA (Linear Discriminant Analysis) in 1D.
