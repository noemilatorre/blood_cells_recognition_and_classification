...
\begin{table}[!ht]
	\begin{NiceTabular}{rX}[rules/color=[gray]{0.9},rules/width=1pt]
		...
		\Body
		\toprule
		\textbf{Trasformazione} & \textbf{Descrizione} \\
		\midrule
		\texttt{HorizontalFlip(p=0.5)} & Ribaltamento orizzontale casuale, utile per simmetrie spaziali. \\
		\texttt{VerticalFlip(p=0.5)} & Ribaltamento verticale per aumentare la varietà. \\
		\texttt{ColorJitter(...)} & Varia casualmente luminosità, contrasto, saturazione e tonalità. \\
		\texttt{RandomBrightnessContrast(p=0.3)} & Modifica casuale della luminosità e del contrasto. \\
		\texttt{MotionBlur(p=0.2)} & Simula sfocatura da movimento. \\
		\texttt{GaussNoise(p=0.2)} & Aggiunge rumore gaussiano. \\
		\texttt{CLAHE(p=0.2)} & Migliora il contrasto locale con equalizzazione adattiva. \\
		\texttt{CoarseDropout(...)} & Oscura aree casuali, simulando occlusioni ottiche. \\
		\texttt{Resize(Config.IMG\_SIZE)} & Ridimensiona le immagini alla dimensione target. \\
		\texttt{Normalize(...)} & Normalizza secondo media e deviazione standard del dataset. \\
		\bottomrule
	\end{NiceTabular}
	\caption{Trasformazioni di \textit{data augmentation} utilizzate nel modello.}
	\label{tab:augmentazioni}
\end{table}


I parametri utilizzati per la normalizzazione RGB, calcolati direttamente sul dataset, sono riportati nella Tabella \ref{tab:normalizzazione}.
...
\begin{table}[!ht]
	\begin{NiceTabular}{rX}[rules/color=[gray]{0.9},rules/width=1pt]
		...
		\Body
		\toprule
		\textbf{Parametro} & \textbf{Valore calcolato sul dataset} \\
		\midrule
		Media RGB & \texttt{[0.7205, 0.7203, 0.7649]} \\
		Deviazione standard RGB & \texttt{[0.2195, 0.2277, 0.1588]} \\
		\bottomrule
	\end{NiceTabular}
	\caption{Parametri di normalizzazione per canale, calcolati sul dataset.}
...
\begin{lstlisting}[caption={Configurazione di CoarseDropout con Albumentations}]
A.CoarseDropout(
